...
class BackgroundPanel(rompanel.ROMPanel):
    ...
    def init(self):
        ...
        #self.rom.data["backgrounds"][0*3].hexlify()
        
        #inst = wx.StaticText(self, -1, "Edit background graphics.")
        #inst.Wrap(inst.GetClientSize()[0])
        
        leftSizer = QVBoxLayout()
        
        #sbs1 = wx.StaticBoxSizer(wx.StaticBox(self, -1, "1. Select a background."), wx.VERTICAL)
        #sbs2 = wx.StaticBoxSizer(wx.StaticBox(self, -1, "Chars/scenes using this background"), wx.VERTICAL)
        ...
        sbs4_widget.setStyleSheet("border: 1px solid black;")  # имитация StaticBox "Edit"
        ...
        """self.backgroundList = wx.ComboBox(self, wx.ID_ANY, size=(200,-1))
        self.backgroundList.AppendItems([bs.name for bs in self.rom.data["backgrounds"]])
        self.backgroundList.SetSelection(0)

        sbs1.Add(self.backgroundList, 0, wx.ALL, 10)
        sbs1.Add((0,10))"""
        ...
        #self.animPanel.refreshSprite(self.rom.data["backgrounds"][0].pixels
        ...
        #self.sizer.AddSizer(sbs6, pos=(1,0), span=(1,2), flag=wx.EXPAND)
        ...
        #wx.EVT_COMBOBOX(self, self.backgroundList.GetId(), self.OnSelectBackground)
    ...
    def printrb(self):
        ...
        #print(self.background.raw_bytes)
    ...
    def refreshPixels(self):
        ...
        #f.write(self.background.raw_bytes)
        #f.close()
        ...
        #self.charList.Clear()
        #self.charList.Append(hex(self.rom.tables["backgrounds"][int(self.background.name.split(" ")[1])*3 + self.side]))
        #self.charList.Append(repr(len(self.background.raw_bytes)))
        #self.charList.Append(repr(len(h)))
        
        #h = h.replace("\n","").replace(" ","").replace(":","")
        
        #f = open("%s %i.txt" % (self.background.name, self.side), "wb")
        #f.write(binascii.unhexlify(h))
        #f.close()
        
        #if self.frame == 0:
        #    self.testPanel.refreshSprite(self.rom.backgroundSubroutine(h).pixels
        #else:
        #    self.testPanel.refreshSprite(self.rom.backgroundSubroutine(h).pixels2
        #self.testPanel.Refresh()
    ...
    def changeBackground(self, num=None):
        ...
        if num is not None:
            ...
            #self.frameList.AppendItems(["Frame %i" % i for i,p in enumerate(self.background.frames)])
            #self.frameList.SetSelection(self.curFrameIdx)
            
            #self.paletteList.Clear()
            #self.paletteList.AppendItems(["Palette %i" % i for i,p in enumerate(self.background.palettes)])

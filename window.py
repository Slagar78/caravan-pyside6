...
class BattleMapViewer(MapViewer):
    def mousePressEvent(self, event: QMouseEvent):
        ...
        # Учитываем прокрутку
        x = event.pos().x() + self.scrollArea.horizontalScrollBar().value()
        y = event.pos().y() + self.scrollArea.verticalScrollBar().value()
        blockX = int(max(0, min(self.map.width-1, (x / obj.scale) / 24)))
        blockY = int(max(0, min(self.map.height-1, (y / obj.scale) / 24)))
